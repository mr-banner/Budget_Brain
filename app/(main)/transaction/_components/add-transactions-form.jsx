  ...
  useEffect(() => {
    if (!transactionLoading && transactionResult) {
      if (transactionResult.success) {
        toast.success( !editMode ? "Transaction created successfully" : "Transaction updated successfully");
        reset();
    ...
    if (scanData) {
      ...
      if (scanData.category) {
        const matchedCategory = categories.find(
          (cat) => cat.name.toLowerCase() === scanData.category.toLowerCase()
        );

        if (matchedCategory) {
          setValue("category", matchedCategory.id, { shouldValidate: true });
        } else {
          console.log("No matching category found for:", scanData.category);
        }
      }
    }
  };
    ...
    <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <Popover>
          <PopoverTrigger asChild className="mt-2">
            ...
            >
              ...
              <CalendarIcon className="ml-auto h-4 w-4 opcity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className={"w-auto p-0"} align="end">
            <Calendar
              ...
              initialFocus
              className="rounded-md shadow-xl/80 shadow-[#d5caff] ring-0 hover:ring-offset-0 focus:ring-0 focus:ring-offset-0"
      ...
      <div className="flex gap-4 space-y-2">
        <Button
          ...
          variant={"outline"}
          onClick={() => router.back()}
          className="flex-1 btn-outline hover:bg-white hover:shadow-[#d5caff]/80 hover:border-[#7f5efd] hover:shadow-md hover:text-[#7f5efd]"
        >
          Cancel
        </Button>
        <Button
        ...
        >
          {transactionLoading ? (
            <>
              <Loader2 className="h-4 w-4 animate-spin mr-2" /> {editMode ? "updating" : "Creating..."}
            </>
          ) : (
