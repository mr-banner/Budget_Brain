  ...
  async ({ step }) => {
    const recurringTransactions = await step.run(
      ...
      async () => {
        ...
        // Fetch only those due today & NOT already processed today
        return await db.transaction.findMany({
          where: {
            ...
            OR: [
              { lastProcessed: null },
              { lastProcessed: { lt: startOfToday } }, // not processed yet today
            ],
          },
        });
      }
  ...
  async ({ event, step }) => {
    ...
    await step.run("process-transaction", async () => {
      ...
      if (
        ...
        transaction.lastProcessed >= startOfToday
      ) {
      ...
      await db.$transaction(async (tx) => {
        ...
        // ✅ Prevent creating duplicate recurring transactions
        const existingToday = await tx.transaction.findFirst({
          where: {
        ...
        if (existingToday) {
          console.log(
            `Skipped duplicate recurring transaction for ${transaction.id}`
          );
          return;
        }
